...
async def commads_start(message: types.Message):
    try:
        ...
        await bot.send_message(message.from_user.id, text="Привет", allow_sending_without_reply=True, reply_markup=kb_clients)
...
async def command_settings(message: types.Message):
    await bot.send_message(message.from_user.id, allow_sending_without_reply=True, text='Настройки', reply_markup=kb_clients_settings)
    #await message.delete()
